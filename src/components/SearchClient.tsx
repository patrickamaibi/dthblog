...
import { formatDate, type PostListItem } from "@/sanity/lib/queries";
import { Search } from "lucide-react";

export default function SearchClient({ posts }: { posts: PostListItem[] }) {
  const [query, setQuery] = useState("");

  const fuse = useMemo(
    () =>
      new Fuse(posts, {
        keys: ["title", "excerpt", "category.title", "author.name", "tags.title"],
        threshold: 0.35,
        includeScore: true,
      }),
    [posts]
            ...
            <article key={post.slug} className={`group py-8 ${i > 0 ? "border-t border-border" : ""}`}>
              ...
              {post.author?.slug && (
                <Link
                  ...
                  className="mt-3 inline-block text-xs font-medium text-muted-foreground hover:text-accent transition-colors"
                >
                  {post.author.name}
                </Link>
              )}
